...
const {User, PostCategories, PostTags, Like, Tags, Categories, Posts} = require("../db/associations")
...
class PostService {
  ...
  async publishPost({Title, Content, user: {UserID}, TagName, CategoryName}) {
    let transaction;
    try {
      // 开启事务
      transaction = await sequelize.transaction();
      // 创建文章
      const post = await Posts.create({Title, Content, UserID, Likes: 0, Views: 0, Replies: 0}, {transaction});
      // 创建或获取标签，并与文章关联
      const tagIDs = await Promise.all(TagName.map(async (TagName) => {
        let tag = await Tags.findOne({where: {TagName}, transaction});
        if (!tag) {
          tag = await Tags.create({TagName}, {transaction});
        }
        await PostTags.create({PostID: post.PostID, TagID: tag.TagID}, {transaction});
        return tag.TagID;
      }));
      ...
      let categoryIDs;
      if (CategoryName) {
        let category = await Categories.findOne({ where: { CategoryName }, transaction });
        if (!category) {
          category = await Categories.create({ CategoryName }, { transaction });
        }
        await PostCategories.create({ PostID: post.PostID, CategoryID: category.CategoryID }, { transaction });
    ...
    try {
      ...
      const topPosts = await Posts.findAll({
        ...
        include: [
          ...
          // {
          //   model: PostTags, // 关联中间表
          //   attributes: ['PostID'] // 不返回中间表的其他属性
          // },
        ]
      });
